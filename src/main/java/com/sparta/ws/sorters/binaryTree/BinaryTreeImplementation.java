...
import com.sparta.ws.exceptions.ChildNotFoundException;
import com.sparta.ws.exceptions.ExceptionConstants;
...
public class BinaryTreeImplementation implements BinaryTree {
    ...
    private void sortAscendingHelper(Node node)
    {
        if(!node.isLeftNodeEmpty())
        {
            sortAscendingHelper(node.getLeftChild());
        }
        sortedTree[index++] = node.getValue();
        if(!node.isRightNodeEmpty())
        {
            sortAscendingHelper(node.getRightChild());
        }
    }
    ...
    private void sortDescendingHelper(Node node)
    {
        if(!node.isRightNodeEmpty())
        {
            sortDescendingHelper(node.getRightChild());
        }
        sortedTree[index++] = node.getValue();
        if(!node.isLeftNodeEmpty())
        {
            sortDescendingHelper(node.getLeftChild());
        }
    }
}
